...
Layer层对象，由对个节点组成
Node节点对象计算和记录自身的信息（比如输出值a、误差项δ等），以及与这个节点相关的上下游的连接
Connection每个连接对象都要记录该连接的权重
Connections仅仅作为Connection的集合对象，提供一些集合操作
'''
#节点类，负责记录和维护及诶单自身信息以及与这个节点相关的上下游连接，实现输出值和误差项的计算
class Node(object):
...
class ConstNode(object):
    ...
    def calc_hidden_layer_delta(self):
        '''
        节点所属隐藏层时，根据式4计算delta
        '''
        downstream_delta = reduce(
            lambda ret,conn: ret + conn.downstream_node.delta * conn.weight,
            self.downstream,0.0)
        self.delta = self.delta * (1 - self.output) * downstream_delta
